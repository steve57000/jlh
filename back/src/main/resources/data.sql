-- ==================================================
-- 0) Sécurité : colonnes de vérification e-mail (PostgreSQL-safe)
-- ==================================================
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'client' AND column_name = 'email_verified'
    ) THEN
        ALTER TABLE client ADD COLUMN email_verified BOOLEAN NOT NULL DEFAULT FALSE;
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'client' AND column_name = 'email_verified_at'
    ) THEN
        ALTER TABLE client ADD COLUMN email_verified_at TIMESTAMP NULL;
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'client' AND column_name = 'vehicule_marque'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'client' AND column_name = 'vehicule_modele'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'administrateur' AND column_name = 'username'
    ) THEN
        ALTER TABLE administrateur ADD COLUMN username VARCHAR(50);
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'administrateur_username_key'
    ) THEN
        ALTER TABLE administrateur ADD CONSTRAINT administrateur_username_key UNIQUE (username);
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'service' AND column_name = 'icon'
    ) THEN
    ...
    IF EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'service' AND column_name = 'icon' AND data_type <> 'text'
    ) THEN
        ALTER TABLE service ALTER COLUMN icon TYPE TEXT;
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'rendez_vous' AND column_name = 'commentaire'
    ) THEN
