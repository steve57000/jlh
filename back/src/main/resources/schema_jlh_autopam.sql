...
CREATE TABLE IF NOT EXISTS client_vehicle (
  ...
  CONSTRAINT fk_client_vehicle_client FOREIGN KEY (id_client)
...
CREATE TABLE IF NOT EXISTS garage_opening_hour (
  id_opening_hour INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  scope VARCHAR(20) NOT NULL,
  status VARCHAR(20) NOT NULL,
  opening_type VARCHAR(20),
  day_of_week VARCHAR(20),
  exceptional_type VARCHAR(20),
  exceptional_date DATE,
  exceptional_start_date DATE,
  exceptional_end_date DATE,
  label VARCHAR(100),
...
CREATE TABLE IF NOT EXISTS service_icon (
  id_icon INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS service (
  ...
  quantite_mode VARCHAR(20) NOT NULL DEFAULT 'UNIQUE',
  prix_mode VARCHAR(20) NOT NULL DEFAULT 'UNITAIRE',
  taille_lot INTEGER,
  quantite_max INTEGER NOT NULL,
  archived BOOLEAN NOT NULL DEFAULT FALSE,
  CONSTRAINT fk_service_icon FOREIGN KEY (id_icon)
    REFERENCES service_icon(id_icon)
);
...
CREATE TABLE IF NOT EXISTS demande_service (
  ...
  quantite_mode_service VARCHAR(20),
  prix_mode_service VARCHAR(20),
  taille_lot_service INTEGER,
...
CREATE TABLE IF NOT EXISTS avis_service (
  ...
  CONSTRAINT fk_avis_service_demande FOREIGN KEY (id_demande)
    REFERENCES demande(id_demande),
  CONSTRAINT fk_avis_service_service FOREIGN KEY (id_service)
    REFERENCES service(id_service),
  CONSTRAINT fk_avis_service_client FOREIGN KEY (id_client)
    REFERENCES client(id_client),
  CONSTRAINT uq_avis_service_demande UNIQUE (id_demande, id_service)
);
...
CREATE TABLE IF NOT EXISTS rendez_vous_proposition (
  id_proposition INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  id_demande INTEGER NOT NULL,
  id_admin INTEGER,
  date_debut TIMESTAMP NOT NULL,
  date_fin TIMESTAMP NOT NULL,
  ...
  CONSTRAINT fk_rdv_prop_demande FOREIGN KEY (id_demande)
    REFERENCES demande(id_demande),
  CONSTRAINT fk_rdv_prop_admin FOREIGN KEY (id_admin)
    REFERENCES administrateur(id_admin)
);
